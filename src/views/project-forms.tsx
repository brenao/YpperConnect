...
export function BaselineDialog({ project }: { project: Project }) {
  ...
  function submit() {
    ...
    toast.success(primeira ? "Baseline registrada" : `Baseline v${anteriores.length + 1} registrada`, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{primeira ? "Registrar baseline do cronograma" : `Nova baseline (v${anteriores.length + 1})`}</DialogTitle>
          <DialogDescription>
            {primeira
        ...
        <div className="grid gap-4">
          <div className="grid gap-2">
            ...
            <Input id="bl-autor" maxLength={80} value={autor} onChange={(e) => setAutor(e.target.value)} />
          </div>
          {!primeira ? (
            <div className="grid gap-2">
          ...
          <p className="rounded-lg border border-border/60 p-3 text-xs text-muted-foreground">
            Serão congeladas {project.tarefas.length} tarefa(s) e o período {project.inicio} a {project.fim}.
          </p>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={() => setOpen(false)}>
